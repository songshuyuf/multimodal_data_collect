...
class Neuracle_HEEG_Device:
    """
    Neuracle HEEG 设备驱动
    直接连接设备IP接收数据
    """

    def __init__(self, hostname='192.168.3.88', port=8172):
        """
        初始化
...
if __name__ == "__main__":
    ...
    def data_callback(data_struct):
        ...
        if packet_count % 10 == 0:  # 每10个包打印一次
    ...
    # 创建设备
    device = Neuracle_HEEG_Device(hostname='192.168.3.88', port=8172)
    ...
    if device.connect():
        ...
        print("\n❌ 无法连接到设备")
        print("\n请检查:")
        print("  1. 设备IP是否正确: 192.168.3.88")
        print("  2. NSH-R软件是否正在采集")
        print("  3. config.ini中DataSend是否=1")
        print("  4. 本地电脑防火墙是否阻止")