...
class NeuracleHEEGDevice:
    ...
    def connect(self, max_retries=3) -> bool:
        ...
        Returns:
            是否成功连接
        """
        self.logger.info(f"正在连接HEEG设备: {self.hostname}:{self.port}")
        ...
        self.logger.error("  2. NSH-R是否已开始采集数据")
        self.logger.error("  3. config.ini中 [DataSend] Value=1")
        return False

    def disconnect(self):
        """断开连接"""
        self.logger.info("正在断开HEEG设备...")
        ...
        self.state = HEEGState.NOT_CONNECTED
        self.logger.info(f"HEEG设备已断开 (接收了 {self.packet_count} 个数据包)")
    ...
    def set_data_callback(self, callback: Callable):
        ...
        Args:
            callback: 回调函数，参数为data_struct字典
        """
    ...
    def start_streaming(self) -> bool:
        ...
        if self.state == HEEGState.NOT_CONNECTED:
            self.logger.error("设备未连接，无法开始数据流")
            return False
        ...
        while True:
            ...
            try:
                ...
                if self.state == HEEGState.CONNECTED:
                    ...
                    self.state = HEEGState.READY
                    self.logger.info(
                        f"HEEG设备就绪: {self.channel_count}通道, "
                        f"{self.sample_rate}Hz"
                    )

                # 更新统计
                self.packet_count += 1
                self.sample_count += data_struct["dataCountPerChannel"]
    ...
    def get_channel_names(self) -> list:
        ...
        Returns:
            通道名称列表 ['EEG1', 'EEG2', ..., 'EEG64', 'Trigger']
        """
        if self.channel_count == 0:
            # 默认64通道
            channels = [f'EEG{i + 1}' for i in range(64)]
        else:
            channels = [f'EEG{i + 1}' for i in range(self.channel_count)]
...
if __name__ == "__main__":
    ...
    print("  1. NSH-R软件正在运行")
    print("  2. 设备状态显示'在线'")
    print("  3. config.ini中 [DataSend] Value=1")
    print("  4. NSH-R正在采集数据")
    print("\n按 Enter 继续...")
    input()
    ...
    def data_callback(data_struct):
        ...
        if packet_count % 100 == 0:  # 每100个包打印一次
            ...
            print(f"\r📊 数据包: {packet_count}, "
                  ...
                  f"通道: {data_struct['channelCount']}, "
                  f"采样率: {data_struct['sampleRate']} Hz", end='')
